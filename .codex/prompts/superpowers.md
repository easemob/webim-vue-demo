...
- SDK 5.0 登录只接受环信 ID 与 Token。NGI 与线上 VIP6 必须使用 SDK DNS 获取与 AppKey 匹配的真实 REST/WS 地址；仅在 TKE/DEV/QA隔舱等用户明确选择的私有化环境传 `serviceConfig.serverUrls` 固定地址，禁止为“可用”而把 Token 强制发送到历史默认集群。固定环境加载已保存配置时，必须先应用当前环境预设；启用 SDK 5.0 联系人或群组同步时，必须传入当前环境真实 `serverUrls.syncWsUrl`。TKE 默认值为 `wss://tke-sdb-fusion.easemob.com/ws`。地址缺失时保留 SDK 同步失败，不改走 REST、DNS 或本地伪造数据。
...
- 已读回执历史选择（覆盖上一条“所有已渲染消息均发送”的通用表述）：接收方点击单聊或群聊时，必须先保存 SDK 5.0 `ConversationItem.readAt` 和 `unreadCount`，再清零未读数。首次历史消息渲染后，只能对 `Message.timestamp > readAt` 的真实 `direct: 'RECEIVE'`、`needReadReceipt: true`、`msgServerId` 消息发送回执；不得拿 `unreadCount` 截取最后 N 条 ID。`readAt` 缺失、非数值或为 `0` 是无法精确选择未读消息 ID 的 SDK / 服务端原始数据，Demo 只输出完整上下文错误和候选消息的原始 ID/时间戳且不猜测 ID；后续只对新渲染消息调用。聊天室不调用。
...
- 单聊和群聊已读回执时机（覆盖上一条“首次入库即回执”的通用表述）：接收端仅在 SDK 5.0 `ChatManager.getCurrentConversation()` 返回与消息相同的 `conversationId`、`conversationType`，且浏览器页面可见时，才对真实 `direct: 'RECEIVE'`、`needReadReceipt: true`、带 `msgServerId` 的当前单聊或群聊消息调用 `sendMessageReadReceipts({ conversationId, conversationType, messageIds })`；首次进入当前单聊或群聊取得的历史消息也遵守此条件。后台、未打开或已切换到其他会话时不发送已读回执，不把“客户端已收消息”伪造成“用户已读”。发送端收到 SDK 5.0 `onMessageReadReceipts` 时仅输出一条 `[Demo <- SDK 5.0 Event]` 原始事件日志，不输出接收端成功或跳过回执的重复 Demo 日志。
- 已读回执创建与接收规则（覆盖上文“仅文本消息”的旧表述）：SDK 5.0 的 `needReadReceipt?: boolean` 属于所有 `CreateMessageBaseParams`。单聊和群聊的 `text/image/file/voice/video/location/cmd/custom/combine` 创建消息必须统一传入 `needReadReceipt: true`；EaseCallKit 直接创建的单聊文本和 CMD 消息也必须传入。该字段不属于 `SendMessageOptions`，不得在 `sendMessage` 或 `ChatClient.sendMessage` 的 options 中透传。接收端只在 SDK 5.0 `getCurrentConversation()` 精确匹配消息的 `conversationId`、`conversationType` 且页面可见时，对真实 `direct: 'RECEIVE'` 的单聊/群聊先调用 `clearConversationUnreadMessageCount({ conversationId, conversationType })`，再仅对 `needReadReceipt: true` 且带 `msgServerId` 的消息调用 `sendMessageReadReceipts({ conversationId, conversationType, messageIds })`。两个 SDK 调用分别保留原始失败，清零失败不得伪造成功或阻断已读回执。聊天室不支持已读回执，创建或接收聊天室消息时均不得调用上述回执 API 或使用本地回执/兼容逻辑代替 SDK 5.0 的真实结果。
...
- 单聊/群聊 Reaction。
- 单聊/群聊消息回执展示只直接消费 SDK 5.0 原始字段：单聊发送方使用 `delivered` 与 `isPeerRead`，群聊发送方使用 `delivered` 与 `groupReadCount`。两者在同一位置将送达展示为绿色 `✓`；单聊收到真实 `isPeerRead === true`、或群聊收到真实 `groupReadCount > 0` 后展示绿色 `✓✓` 并覆盖送达图标；群聊继续展示真实已读人数。接收方不得显示对方消息的回执图标，禁止通过本地状态、旧字段或兼容映射推断已读。
