...
- IM 能力唯一依赖本地 `easemob-websdk 5.0.2`；初始化使用 `ChatClient` 与领域 Manager，禁止恢复 SDK 4.0 客户端实例、私有子路径或兼容层。
