...
export const imContactListener = () => {
  const submitInformData = (eventName, payload) => {
    ...
    Promise.resolve(store.dispatch('createNewInform', { eventName, payload })).catch(
      (error) => console.error('[imContactListener.createNewInform]', error),
    );
  };

  const syncContactsFromSdkSnapshot = (eventName) => {
    Promise.resolve(store.dispatch('syncContactsFromSdkSnapshot')).catch((error) =>
      console.error(`[SDK 5.0 Contact Event] ${eventName} snapshot sync failed`, error),
    );
  };
