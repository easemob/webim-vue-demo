...
当前 SDK 包：`easemob-websdk 5.0.2`，来源为 `node_modules/easemob-websdk/dist/*.d.ts`。

本次升级已对比 `easemob-websdk 5.0.1` 与 `5.0.2` 的 `.d.ts` 类型声明内容哈希，公开类型声明一致，因此当前静态覆盖率分母 / 分子保持不变。
...
| 已覆盖公开对外 API | 189 | 仅统计 Demo 运行代码中的真实 SDK 5.0 调用，不包含 `tests/` |
| 未覆盖公开对外 API | 13 | 仅包含当前 `src/` 没有用户可见入口或真实调用的公开对外 API |
...
| 分类 | 数量 | 说明 |
| --- | ---: | --- |
| 公开对外 API | 202 | 本文主矩阵按功能归类覆盖，覆盖率按 API 名去重统计 |
...
| 设备资源与 RTC | `ChatClient.getClientResource`, `ChatClient.getRTCTokenInfo`, `ChatClient.getUserIdsWithRTCUids`, `ChatClient.getSelfIdsOnOtherPlatform` | 部分覆盖 | EaseCallKit 使用 `getClientResource` 和 `getRTCTokenInfo`。 | 未覆盖 RTC UID 反查 IM 用户、查询其他平台登录 ID。 |
...
| 翻译和语音转文字 | `ChatManager.getSupportedTranslationLanguages`, `translateMessage`, `voiceMessageToText` | 是 | 文本消息右键菜单已读取 SDK 5.0 支持语言列表，并调用 `translateMessage({ message, targetLanguages })` 展示真实翻译结果；语音消息右键菜单已调用 `voiceMessageToText(message.body)` 展示真实转写结果。 | 按当前验收口径，语音转文字已覆盖，不再新增本地语音文件入口。 |
...
- 当前静态统计：公开对外 API 共 `202` 个，已覆盖 `189` 个，仍有 `13` 个未覆盖；`@internal` 私有入口已剔除，不计入分母或缺口。
- 消息：附件下载、群消息已读用户 / 回执详情、置顶消息列表、全会话未读清零、清空全部消息与会话、当前会话上下文、主动刷新会话列表、消息事件监听移除已补齐；语音转文字按当前验收口径已由已有语音消息 `voiceMessageToText(message.body)` 覆盖。仍未覆盖 `ChatManager.voiceFileToText`（`1` 个）。
- ChatClient：连接状态查询、REST 上下文、Token 续期、缓存管理、上传适配器、联系人快照和事件移除已覆盖；仍未覆盖动态 `ChatClient.use` 注册、`ChatClient.getUserIdsWithRTCUids` RTC UID 反查、`ChatClient.getSelfIdsOnOtherPlatform` 其他平台登录 ID 查询（`3` 个）。
...
- 群组：公开 `Group` facade 已覆盖本地轻量摘要、详情、`refresh`、资料修改、配置更新、群主转让、退群、解散、成员、管理员、黑名单、白名单、禁言、公告、共享文件和群成员属性 / 群名片；`Group.getSummary()` 按 SDK 真实结果展示 `JoinedGroupSummary | null`，不通过 `getDetail` 本地伪造摘要。所有 `@internal` Manager 转发入口均不在公开覆盖分母或未覆盖清单中。
...
- 缺口合计：`1`（消息）+ `3`（ChatClient）+ `2`（平台适配）+ `7`（消息话题 Manager 直调）= `13`。前 `6` 个是尚未接入的独立页面能力；后 `7` 个已有等价 `ChatThread` facade 用户能力，属于为保持 SDK 5.0 单路径而保留的静态 API 缺口。
