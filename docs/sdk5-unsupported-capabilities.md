...
| 举报消息 | 消息右键“举报”弹窗 | `ChatManager` 公开 API 不含 `reportMessage` | 提交时显示 `SDK 5.0 current package does not expose reportMessage; no fallback is configured.`，不显示成功 | 已确认不支持 |
