...
本文件记录当前 Demo 的 `easemob-websdk 5.0` 没有公开 API，或真实 SDK / 服务端行为与公开契约不一致的现有 Demo 功能。
...
| --- | --- | --- | --- | --- |
| 解散聊天室 | Demo 无入口 | `ChatRoomManager` 公开 API 不含 `destroyChatRoom` | Demo 不提供入口，不调用私有 API、REST 或本地模拟 | 已确认不支持 |
...
| 聊天室合并消息下行 | 聊天室输入框“发送合并消息” | `ChatManager.createCombineMessage` 的类型声明允许 `conversationType: 'chatRoom'`，但历史 NGI 聊天室实测中发送端返回成功，接收事件实际为 `type: 'text'`，内容为 SDK 的兼容文本“版本过低”，而非 `type: 'combine'`；当前环境需重新实测 | 页面如实展示 SDK 下行的文本兼容内容；不把文本伪造为合并消息 | 历史实测不通过；当前环境待复测 |
| EaseCallKit 查询频道成员 | EaseCallKit 多人通话频道成员展示 | `ChatClient` 公开 `getRTCTokenInfo` 与 `getUserIdsWithRTCUids`，但不公开按频道名查询成员列表的 API；旧实现直接请求 REST `/channel/mapper` | 组件打印并抛出 `WebSDK 5.0 does not expose a channel-member query API for EaseCallKit.`；不再调用旧 REST 客户端或伪造成员列表 | 已确认不支持 |
