...
本文件与 [`sdk5-unsupported-capabilities.md`](./sdk5-unsupported-capabilities.md) 分开维护：这里只记录当前本地 `easemob-websdk 5.0.0` 已公开、可由 SDK 5.0 真实调用，但尚未在页面实现且未写入 `cases_list.md` 的用户可见能力。
...
| 清除服务端会话未读数 | `chatManager.clearConversationUnreadMessageCount` | 当前“清空当前会话消息展示”是本地展示操作，未调用 SDK 的服务端会话未读清零能力 | 待实现 |
